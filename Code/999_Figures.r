    ...
    df_f <- mcn %>% filter(mean_mangrove_area>1 & 
                                is.finite(lag_gap_density_avg) &
                               is.finite(logGDPpc))
    scen$gdppc5_capped <- matrixStats::rowMins(cbind(scen$gdppc5,rep(max(exp(df_f$logGDPpc),na.rm=TRUE),times=length(scen$gdppc5))))
    
    scen_2100 <- scen %>% filter(year==2100)
    
    max(df_f$logGDPpc)
    scen %>% filter(is.finite(gdppc5)) %>% summarize(max_gdppc5 = log(max(gdppc5,na.rm=TRUE)), max_capped =  log(max(gdppc5_capped,na.rm=TRUE)))
    max(scen$gdppc5,na.rm=TRUE)
## Map
#### Gridcells
###### Slopes
    ...
    oceans <- read_sf("G:\\My Drive\\Data\\Oceans\\ne_50m_ocean.shp")
...
# Convert to sf object using the 'geom' column
data_slopes_sf <- st_as_sf(data_slopes, sf_column_name = "geom", crs = 3857)  # Replace 3857 with your actual CRS if different

# Save to shapefile
#st_write(data_slopes_sf, "data_slopes_output.shp")
    ...
    class(data_slopes)
        ...
        ### To send it to shapefile
            library(sf)
            data_slopes_ha <- data_slopes %>% dplyr::select(gridcell_id,slope_area,R5,X2013,X2014,X2015,X2016,X2017,X2018,X2019,X2020,X2021,geom)
            glimpse(data_slopes_ha )
            # Write the sf object to a Shapefile
            #st_write(data_slopes_ha, "esri_files/data_slopes_v2.shp")
        ###
    glimpse(data_slopes)
    mangrove_map_outofsample <- ggplot() +
        geom_sf(data = oceans_robin, fill = "black",alpha=0.85) +
        geom_sf(data = world, color = "gray40", fill = "gray40") +
        geom_sf(data = data_slopes %>% filter(gridcell_id %in% gridcells_hotter$gridcell_id), mapping = aes(geometry = geom), color="red", fill="red", 
            size = 1000000, show.legend = TRUE, inherit.aes = FALSE) +
            geom_sf(data = data_slopes %>% filter(gridcell_id %notin% gridcells_hotter$gridcell_id), mapping = aes(geometry = geom), color="darkcyan", fill="darkcyan", 
            size = 1000000, show.legend = TRUE, inherit.aes = FALSE) +
        coord_sf(crs = "+proj=robin", ylim = c(-39*10^5, 30*10^5))+
    ...
    theme(legend.position = "bottom",
            ...
            mangrove_map_outofsample
            plot_sst_capped
    ggarrange(plot_sst_capped,mangrove_map_outofsample,ncol=1)
    #ggsave()
    ...
    #ggsave("Figures/Draft4/SM_outofsample.png",dpi=600)
...
sum_area %>%summarize(change = cumsum(area_change_sum))
...
###### Slopes
 ...
 # Combine the combined_plot with other plots
    ...
    final_plot
    #ggsave("Figures/Draft2/Fig1_draft3.png",dpi=600)
## Models
#### Area
###### Effects
                ...
                Models_ssthot_plot_gdp <- ggplot(sq_estimate_gdp_area)+
                    ...
                    geom_ribbon(aes(x=temp,ymin=ci1,ymax=ci2),fill="#25625f",alpha=0.2)+
                        geom_hline(aes(yintercept=0),linetype="dashed")+
                        ylab("Effect of 1% Increase (%)")+xlab("Log GDP per capita***")+theme_minimal()+ xlim(c(0,13.9))
                       
                Models_ssthot_plot_gdp

                max(log(scen_2100$gdppc5_capped),na.rm=TRUE)
                histogram_plot_gdp <- ggplot(mcn_2020, aes(x = logGDPpc)) +
                        geom_histogram(aes(y = ..density..), colour = "black", fill = "#25625f") +
                        geom_density(data=scen_2100,aes(log(gdppc5_capped)),color="indianred",alpha = .2, size=1.3) +
                        #geom_density(alpha = .2, fill = "#FF6666") +
                        theme_bw() +
                        xlab("Log GDP per capita***") +
                        ylab("Density") + theme_minimal() + xlim(c(0,13.9))
                
                Models_ssthot_plot_gdp <- ggarrange(Models_ssthot_plot_gdp, histogram_plot_gdp,
                legend="none", ncol = 1,heights=c(1,1),align="hv", hjust=0)
                Models_ssthot_plot_gdp
                ...
                histogram_plot_ssthot <- ggplot(mcn_2020, aes(x = sst_hottest)) +
                        geom_histogram(aes(y = ..density..), colour = "black", fill = "#e9995c") +
                        geom_density(data=scen_2100,aes(sst_hot_70_capped),color="indianred",alpha = .2, size=1.3) +
                        #geom_density(alpha = .2, fill = "#FF6666") +
                        theme_bw()+xlim(c(18,35)) +
                        xlab("Mean SST in the Hottest Month (°C)***") +
                        ylab("Density") + theme_minimal()
                
                Models_ssthot_plot_area<-ggarrange(Models_ssthot_plot_area, histogram_plot_ssthot,legend="none", 
                ncol = 1,heights=c(1,1),align="hv", hjust=0)
                Models_ssthot_plot_area
               ...
               #ggsave("Figures/Draft3/AreaModel_MarginalEffect_SSP570.png",dpi=600)
...
## Projection
    #### Area
        ...
        ## ONLY CLIMATE EFFECT (start)
            ...
            ######## By Country
            ######## and by Region
                    scen_arealoss_perc_onlyCC <- read.csv(file="Results\\Area\\Proj_Area_Perc_OnlyCC_SSP570_Sept2024.csv") #read.csv(file="Results\\Area\\scen_arealoss_perc_onlyCC.csv") 
                    ...
                    agg_aloss_neg_onlyCC <-  read.csv(file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP570_Sept2024.csv") #read.csv(file="Results\\Area\\agg_aloss_neg_onlyCC.csv")
                    agg_aloss_neg_total_onlyCC <- read.csv(file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP170_Sept2024.csv") #  read.csv(file="Results\\Area\\agg_aloss_neg_total_onlyCC.csv")
                    agg_loss_by_country <-  read.csv(file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP170_Sept2024.csv") #read.csv(file="Results\\Area\\agg_loss_by_country.csv")
                    load(file="Results\\Area\\map_aloss_neg_country_onlyCC.Rda")
                    write.csv(scen_arealoss_perc_onlyCC_2100,"Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP570_July2025.csv")
            ...
            ######## Total with Historical
                ...
                agg_aloss_neg_total_onlyCC <- read.csv(file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP570_Sept2024.csv") 
                ggplot()+
                    # geom_bar(aes(x=(year),y=-mangrove_area_future_loss,
                    #     fill=factor(R5)),stat="identity")+
                    # xlab("Year")+ylab("Area Change \n(km2)")+
        ...
        ## Only GDP Effect (start)
            ...
            agg_aloss_neg_total_onlygdp <- read.csv(file="Results\\Area\\Proj_Area_Perc_Total_OnlyGDP_SSP570_Sept2024.csv")
            ggplot()+
                    # geom_bar(aes(x=(year),y=-mangrove_area_future_loss,
                    #     fill=factor(R5)),stat="identity")+
                    # xlab("Year")+ylab("Area Change \n(km2)")+
        ...
        ## Both (start)
            ...
            agg_aloss_neg_total_both <- read.csv(file="Results\\Area\\Proj_Area_Perc_Total_both_SSP270.csv")
            agg_aloss_neg_total_both %>% filter(year==2100)
            
            agg_aloss_neg_total_both <- read.csv(file="Results\\Area\\Proj_Area_Perc_Total_both_SSP570_Sept2024.csv")
            agg_aloss_neg_total_onlyCC <- read.csv(file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP570_Sept2024.csv")
            agg_aloss_neg_total_onlygdp <- read.csv(file="Results\\Area\\Proj_Area_Perc_Total_Onlygdp_SSP570_Sept2024.csv")
            ...
            agg_aloss_neg_total_onlyCC %>% filter(year==2100)
            ...
            area_change_both_plot <- ggplot()+
                    ...
                    #ggsave("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Figures\\Draft3\\Projection_Area_SSP570.png")

        ## Both (end)

        ## By continents (start)
            scen_arealoss_perc_onlygdp_countrycode <- read.csv(file="Results\\Area\\Proj_Area_Perc_Country_Onlygdp_SSP570_Sept2024.csv")
            scen_arealoss_perc_onlyCC_countrycode <- read.csv(file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP570_Sept2024.csv")
            scen_arealoss_perc_both_countrycode <- read.csv(file="Results\\Area\\Proj_Area_Perc_Country_both_SSP570_Sept2024.csv")
...
## Benefit Losses (start)
    ...
    # Benefit Losses in Gridcells
        ...
        scen_allforcings_allES %>% filter(year %in% c(2100,2080,2050),countrycode=="NZL",type=="cultural") %>% select(year,marg_ssth_area,sst_hot_70)
        scen$marg_ssth_area <-  coef_ssth + 2*coef_ssth2*scen$sst_hot_70
        ggplot() + 
            geom_point(data=scen_allforcings_allES %>% filter(year %in% c(2100,2080,2050),countrycode=="NZL",type=="cultural"),aes(x=sst_hot_70,
                            #color=log(benefit_change/10^6),
                            color=forcing,shape=factor(year),y=marg_ssth_area)) + theme_bw() + 
                            geom_vline(aes(xintercept=27.546), linetype=2, color="indianred") + 
                            geom_hline(aes(yintercept=0), linetype=2, color="black")+
                            geom_text(aes(x=27.546, y=0.004) ,label="A",color="indianred") 
        ggplot() + 
            geom_bar(data=scen_allforcings_allES %>% filter(year %in% c(2100,2080,2050),countrycode=="NZL",type=="cultural"),aes(x=sst_hot_70,
                            #color=log(benefit_change/10^6),
                            fill=factor(year),y=marg_ssth_area*100), stat="identity",width=0.1,alpha=0.5) + theme_bw() +
                                geom_vline(aes(xintercept=27.546), linetype=2, color="indianred") + 
                            geom_hline(aes(yintercept=0), linetype=1, color="black")+
                            geom_text(aes(x=26.7, y=0.012*100) ,label="Temperature of\nInflection (27.5C)",color="indianred") + 
                            labs(fill="Year",title="Marginal Effect of Temperature in New Zealand's Mangroves\n for selected years under RCP7.0", x="SST of the Warmest Month", y="Marginal Effect \n(Percent Change in Mangrove Area)")
        
    ggsave("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Figures\\Submission2\\Marg_Effect_NZL.png",dpi=600) #Submission 2 Final Figure
        ...
        mex_gridcell_loss_log <- ggplot(data = scen_allforcings_allES_dif %>% filter(year %in% c(2100), countrycode == "MEX")) + 
            ...
            labs(x = "Log local GDP per capita", y = "Foregone Benefits in 2100\n (million USD)",title="Climate Damages in 2100") + 
    ...
    benefit_loss_2100_total <- ggplot(diff_country_total %>% filter(year %in% c(2100)))+
        geom_text(data=diff_country_total%>% 
                                filter(year %in% c(2100), benefit_change<0) ,
                                aes(x=GDP_Country_ssp5*1.62,
                                y=-benefit_change/10^6,label=countrycode),alpha=0.9,size=2.9)+
        geom_point(aes(x=(GDP_Country_ssp5*1.62), y=-benefit_change/10^6, color = R5,alpha=0.5)) + 
        scale_color_scico_d(palette = "batlow",end=0.9,begin=0.1)+
        labs(color="Region",y="Foregone Benefits in 2100 \n(Million 2020 USD)",x="Country-level GDP (Billion 2020 USD)",title="Country-level Damages in 2100")+
        scale_y_continuous(trans="log10", labels = label_number(scale = 1, accuracy = 0.01)) +   
    ...
    ggarrange(mex_gridcell_lines+labs(title="A. Mangrove Trajectories\n in Mexico"),
                        mex_gridcell_loss_log+labs(title="B. Foregone Benefits\n in Mexico"),
                        benefit_loss_2100_total+labs(title="C. Foregone Benefits Globally")+
            guides(colour = guide_legend(title.position="top", title.hjust = 0.5))+coord_flip(),legend="bottom",ncol=3,widths=c(2,2,3))
    
    ggsave("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Figures\\Submission2\\Path_Mexico2100_Global2100_v3.png",dpi=600) #Submission 2 Final Figure
    ...
    scale_fill_scico(palette = "lajolla", breaks = log_breaks, direction=-1,
        labels = comma(actual_breaks),na.value="transparent",
        begin=0.1,end=0.9) +
    labs(fill = "Foregone Benefits \n(Million USD/yr/C)")+
    ...
    theme_minimal()+
        ...
        gg
        ggsave("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Figures\\Submission2\\Benefit_USD_MAP_SSP370_Branderv2.png",dpi=600)
