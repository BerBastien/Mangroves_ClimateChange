    ...
    # scen_arealoss_perc_onlyCC <- read.csv(file="Results\\Area\\scen_arealoss_perc_onlyCC_ssp370.csv")
    # scen_arealoss_perc_onlygdp <- read.csv(file="Results\\Area\\scen_arealoss_perc_onlygdp_ssp370.csv")
    # scen_arealoss_perc_both <- read.csv(file="Results\\Area\\scen_arealoss_perc_both_ssp370.csv")
    ...
    scen_allforcings_allES_country <- scen_allforcings_allES %>% group_by(countrycode,year,forcing,type) %>%
                    summarise(
                        ...
                        #GDP_Country_ssp370 = first(GDP_Country_ssp370),
                        mangrove_area_future_loss = sum(mangrove_area_future_loss,na.rm=TRUE),
                        mangrove_area = sum(mangrove_area,na.rm=TRUE)
                    ) %>%
                    ungroup()

    scen_allforcings_allES_country %>% filter(type=="water",year==2100 & forcing=="both" & GDP_SSP3>0 & benefit_future<1) %>%as.data.frame()
    ...
    BenefitChange_2100
    write.csv(diff_df_country_type,file="Data/projections/diff_df_country_type_ssp370.csv")
    ...
    diff_df_country <- diff_df_country_type %>% group_by(year,countrycode) %>%
                        summarise(benefit_change_provisioning = diff_benefit_change[type=="wood"] + diff_benefit_change[type=="fisheries"], 
                                    ...
                                    R5 = R5[type=="water"],
                                    POP_SSP3 = POP_SSP3[type=="water"],
                                    GDP_SSP3 = GDP_SSP3[type=="water"],
    ...
    diff_country_total <- long_df %>% group_by(year,countrycode) %>%
                        summarise(benefit_change = benefit_change[type=="provisioning"] + benefit_change[type=="regulating"], 
                                    ...
                                    POP_SSP3 = POP_SSP3[type=="provisioning"],
                                    GDP_SSP3 = GDP_SSP3[type=="provisioning"],
