#SSP1
        ...
        mcn <- read.csv("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Data\\mangrove_alldata_sept2023.csv")
                        ...
                        scen <- merge(scen,mcn %>% filter(year==2022) %>% dplyr::select("gridcell_id","sst_hottest","logGDPpc"),by="gridcell_id",all=TRUE)
                        scen <- scen %>% 
                                group_by(gridcell_id) %>% 
                                mutate(sst_hot_70_releveled = sst_hot_70 - (sst_hot_70[year==2022][1]-sst_hottest[year==2022][1])) %>% 
                                ungroup()
                        glimpse(scen)
                        scen$sst_hot_70_capped <- matrixStats::rowMins(cbind(scen$sst_hot_70_releveled,rep(max(mcn$sst_hottest,na.rm=TRUE),times=length(scen$sst_hot_70))))

                        max(mcn$year)
                        
                     plot_sst_capped <- ggplot(scen %>% filter(year>2021)) + 
                        geom_line(aes(x=year, y=sst_hot_70_releveled, group=gridcell_id, col="Projected (out of sample)")) + 
                        geom_line(aes(x=year, y=sst_hot_70_capped, group=gridcell_id, col="Projected (within sample)")) +  
                        geom_line(data = mcn, aes(x=year, y=sst_hottest, group=gridcell_id, col="Historical")) +
                        scale_color_manual(values=c("Projected (out of sample)"="red", "Projected (within sample)"="darkcyan", "Historical"="black")) +
                        labs(x = "year", y="SST of the Hottest Month (C)",col="")+
                        theme_minimal()
                        plot_sst_capped

                        glimpse(scen)
                        gridcells_hotter <- scen %>% filter(year==2100,sst_hot_70_releveled!=sst_hot_70_capped) %>% select(gridcell_id)
                        
                        scen %>% mutate(dif = sst_hot_70_releveled-sst_hot_70_capped) %>% 
                        mutate(group_sample=ifelse(dif==0,"within","outside"))%>% 
                        group_by(group_sample) %>% 
                        summarize(count = n())
                        792/132056
                        #ggsave("Figures/Response2Rev/Capped_Temp.png",dpi=600)
                        
                        scen$marg_ssth_area <-  coef_ssth + 2*coef_ssth2*scen$sst_hot_70_capped
                        scen$marg_ssth_area_nocapped <-  coef_ssth + 2*coef_ssth2*scen$sst_hot_70_releveled
                        ggplot(scen)+geom_point(aes(y=marg_ssth_area,x=marg_ssth_area_nocapped))
                        ...
                        scen$gdppc1_capped <- matrixStats::rowMins(cbind(scen$gdppc1,rep(max(exp(scen$logGDPpc),na.rm=TRUE),times=length(scen$gdppc1))))
                        scen$gdppc5_capped <- matrixStats::rowMins(cbind(scen$gdppc5,rep(max(exp(mcn$logGDPpc),na.rm=TRUE),times=length(scen$gdppc5))))
                        #max((scen$gdppc1[which(is.finite(scen$gdppc1))]),na.rm=TRUE)
                        
                        #ggplot(scen)+geom_point(aes(x=log(gdppc1),y=log(gdppc1_capped)))

                        plot_gdp_capped <- ggplot(scen %>% filter(year>2020)) + 
                        geom_line(aes(x=year, y=log(gdppc5), group=gridcell_id, col="Projected (out of sample)")) + 
                        geom_line(aes(x=year, y=log(gdppc5_capped), group=gridcell_id, col="Projected (within sample)")) +  
                        geom_line(data = mcn, aes(x=year, y=logGDPpc, group=gridcell_id, col="Historical")) +
                        scale_color_manual(values=c("Projected (out of sample)"="red", "Projected (within sample)"="darkcyan", "Historical"="black")) +
                        labs(x = "year", y="Log GDP per capita",col="")+
                        theme_minimal()
                        plot_gdp_capped 
                        #ggsave("Figures/Response2Rev/Capped_Temp.png",dpi=600)

                        scen %>% mutate(dif = gdppc5-gdppc5_capped) %>% 
                        mutate(group_sample=ifelse(dif==0,"within","outside"))%>% 
                        group_by(group_sample) %>% 
                        summarize(count = n())
                        4196/112254
                        ...
                        write.csv(scen_arealoss_perc_onlyCC, file="Results\\Area\\Proj_Area_Perc_OnlyCC_SSP170_Sept2024.csv")
                        ...
                        write.csv(scen_arealoss_perc_onlyCC_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP170_Sept2024.csv")
                                ...
                                write.csv(agg_aloss_neg_total_onlyCC, file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP170_Sept2024.csv")
                        ...
                        write.csv(scen_arealoss_perc_onlygdp, file="Results\\Area\\Proj_Area_Perc_OnlyGDP_SSP170_Sept2024.csv") 
                        ...
                        scen_arealoss_perc_onlygdp_countrycode <- scen_arealoss_perc_onlygdp %>% group_by(countrycode,year) %>% 
                                                                ...
                                                                mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                                mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                        write.csv(scen_arealoss_perc_onlygdp_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyGDP_SSP170_Sept2024.csv") 
                        ...
                        sum2020 <- sum(mcn_2020$mangrove_area,na.rm=T)
                                ...
                                write.csv(agg_aloss_neg_total_onlygdp, file="Results\\Area\\Proj_Area_Perc_Total_OnlyGDP_SSP170_Sept2024.csv")
                        ...
                        write.csv(scen_arealoss_perc_both, file="Results\\Area\\Proj_Area_Perc_both_SSP170_Sept2024.csv") 
                        ...
                        scen_arealoss_perc_both_countrycode <- scen_arealoss_perc_both %>% group_by(countrycode,year) %>% 
                                                                ...
                                                                mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                                mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                        write.csv(scen_arealoss_perc_both_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_both_SSP170_Sept2024.csv")
                        ...
                        ## New Figure Pas + Future (start)area_change_wrt_1996_lower_limit
                                ...
                                write.csv(agg_aloss_neg_total_both, file="Results\\Area\\Proj_Area_Perc_Total_both_SSP170_Sept2024.csv")
...
## SSP2
    ...
    mcn <- read.csv("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Data\\mangrove_alldata_sept2023.csv")
                ...
                scen$sst_hot_70_capped <- matrixStats::rowMins(cbind(scen$sst_hot_70,rep(max(scen$sst_hottest,na.rm=TRUE),times=length(scen$sst_hot_70))))
                ...
                ## Only Climate Change (start)
                    ...
                    scen_arealoss_perc_onlyCC <- scen_arealoss_perc_onlyCC %>% filter(!is.na(gridcell_id))
                    glimpse(scen_arealoss_perc_onlyCC)
                    write.csv(scen_arealoss_perc_onlyCC, file="Results\\Area\\Proj_Area_Perc_OnlyCC_SSP270_Sept2024.csv")
                    ...
                    scen_arealoss_perc_onlyCC_countrycode <- scen_arealoss_perc_onlyCC %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlyCC_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP270_Sept2024.csv")
                        ...
                        write.csv(agg_aloss_neg_total_onlyCC, file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP270_Sept2024.csv")
                ...
                ## Only GDP (start)
                    ...
                    glimpse(scen_arealoss_perc_onlygdp)
                    scen_arealoss_perc_onlygdp <- scen_arealoss_perc_onlygdp %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_onlygdp, file="Results\\Area\\Proj_Area_Perc_OnlyGDP_SSP270_Sept2024.csv") 
                    agg_aloss_neg_total_onlygdp <- aggregate(mangrove_area_future_loss~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_lower_limit <- aggregate(mangrove_area_future_loss_lower_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_upper_limit <- aggregate(mangrove_area_future_loss_upper_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp <- merge(agg_aloss_neg_total_onlygdp,agg_aloss_neg_total_onlygdp_upper_limit,by="year")
                    ...
                    scen_arealoss_perc_onlygdp_countrycode <- scen_arealoss_perc_onlygdp %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlygdp_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyGDP_SSP270_Sept2024.csv") 
                    glimpse(scen_arealoss_perc_onlygdp_countrycode)
                    ...
                    sum2020 <- sum(mcn_2020$mangrove_area,na.rm=T)
                        ...
                        write.csv(agg_aloss_neg_total_onlygdp, file="Results\\Area\\Proj_Area_Perc_Total_OnlyGDP_SSP270_Sept2024.csv")
                ...
                ## Both: Climate and Socioeconomic (start)
                    ...
                    glimpse(scen_arealoss_perc_both)
                    scen_arealoss_perc_both <- scen_arealoss_perc_both %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_both, file="Results\\Area\\Proj_Area_Perc_both_SSP270_Sept2024.csv") 
                    ...
                    scen_arealoss_perc_both_countrycode <- scen_arealoss_perc_both %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_both_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_both_SSP270_Sept2024.csv")
                    glimpse(scen_arealoss_perc_both_countrycode)
                    ...
                    ## New Figure Pas + Future (start)area_change_wrt_1996_lower_limit
                        ...
                        #write.csv(summary_dfs_total_sum,file="Results\\Area\\summary_dfs_total_sum.csv")
                        write.csv(agg_aloss_neg_total_both, file="Results\\Area\\Proj_Area_Perc_Total_both_SSP270_Sept2024.csv")
...
## SSP4
    ...
    mcn <- read.csv("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Data\\mangrove_alldata_sept2023.csv")
                ...
                ## Only Climate Change (start)
                    ...
                    scen_arealoss_perc_onlyCC <- scen_arealoss_perc_onlyCC %>% filter(!is.na(gridcell_id))
                    glimpse(scen_arealoss_perc_onlyCC)
                    write.csv(scen_arealoss_perc_onlyCC, file="Results\\Area\\Proj_Area_Perc_OnlyCC_SSP470_Sept2024.csv")
                    ...
                    scen_arealoss_perc_onlyCC_countrycode <- scen_arealoss_perc_onlyCC %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlyCC_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP470_Sept2024.csv")
                        ...
                        write.csv(agg_aloss_neg_total_onlyCC, file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP470_Sept2024.csv")
                ...
                ## Only GDP (start)
                    ...
                    glimpse(scen_arealoss_perc_onlygdp)
                    scen_arealoss_perc_onlygdp <- scen_arealoss_perc_onlygdp %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_onlygdp, file="Results\\Area\\Proj_Area_Perc_OnlyGDP_SSP470_Sept2024.csv") 
                    agg_aloss_neg_total_onlygdp <- aggregate(mangrove_area_future_loss~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_lower_limit <- aggregate(mangrove_area_future_loss_lower_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_upper_limit <- aggregate(mangrove_area_future_loss_upper_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp <- merge(agg_aloss_neg_total_onlygdp,agg_aloss_neg_total_onlygdp_upper_limit,by="year")
                    ...
                    scen_arealoss_perc_onlygdp_countrycode <- scen_arealoss_perc_onlygdp %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlygdp_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyGDP_SSP470_Sept2024.csv") 
                    glimpse(scen_arealoss_perc_onlygdp_countrycode)
                    ...
                    sum2020 <- sum(mcn_2020$mangrove_area,na.rm=T)
                        ...
                        write.csv(agg_aloss_neg_total_onlygdp, file="Results\\Area\\Proj_Area_Perc_Total_OnlyGDP_SSP470_Sept2024.csv")
                ...
                ## Both: Climate and Socioeconomic (start)
                    ...
                    glimpse(scen_arealoss_perc_both)
                    scen_arealoss_perc_both <- scen_arealoss_perc_both %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_both, file="Results\\Area\\Proj_Area_Perc_both_SSP470_Sept2024.csv") 
                    ...
                    scen_arealoss_perc_both_countrycode <- scen_arealoss_perc_both %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_both_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_both_SSP470_Sept2024.csv")
                    glimpse(scen_arealoss_perc_both_countrycode)
                    ...
                    ## New Figure Pas + Future (start)area_change_wrt_1996_lower_limit
                        ...
                        #write.csv(summary_dfs_total_sum,file="Results\\Area\\summary_dfs_total_sum.csv")
                        write.csv(agg_aloss_neg_total_both, file="Results\\Area\\Proj_Area_Perc_Total_both_SSP470_Sept2024.csv")
...
## SSP5
    ...
    mcn <- read.csv("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Data\\mangrove_alldata_sept2023.csv")
                ...
                ## Only Climate Change (start)
                    ...
                    scen_arealoss_perc_onlyCC <- scen_arealoss_perc_onlyCC %>% filter(!is.na(gridcell_id))
                    glimpse(scen_arealoss_perc_onlyCC)
                    write.csv(scen_arealoss_perc_onlyCC, file="Results\\Area\\Proj_Area_Perc_OnlyCC_SSP570_Sept2024.csv")
                    ...
                    scen_arealoss_perc_onlyCC_countrycode <- scen_arealoss_perc_onlyCC %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlyCC_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP570_Sept2024.csv")
                        ...
                        write.csv(agg_aloss_neg_total_onlyCC, file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP570_Sept2024.csv")
                ...
                ## Only GDP (start)
                    ...
                    glimpse(scen_arealoss_perc_onlygdp)
                    scen_arealoss_perc_onlygdp <- scen_arealoss_perc_onlygdp %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_onlygdp, file="Results\\Area\\Proj_Area_Perc_OnlyGDP_SSP570_Sept2024.csv") 
                    agg_aloss_neg_total_onlygdp <- aggregate(mangrove_area_future_loss~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_lower_limit <- aggregate(mangrove_area_future_loss_lower_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_upper_limit <- aggregate(mangrove_area_future_loss_upper_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp <- merge(agg_aloss_neg_total_onlygdp,agg_aloss_neg_total_onlygdp_upper_limit,by="year")
                    ...
                    scen_arealoss_perc_onlygdp_countrycode <- scen_arealoss_perc_onlygdp %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlygdp_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyGDP_SSP570_Sept2024.csv") 
                    glimpse(scen_arealoss_perc_onlygdp_countrycode)
                    ...
                    sum2020 <- sum(mcn_2020$mangrove_area,na.rm=T)
                        ...
                        write.csv(agg_aloss_neg_total_onlygdp, file="Results\\Area\\Proj_Area_Perc_Total_OnlyGDP_SSP570_Sept2024.csv")
                ...
                ## Both: Climate and Socioeconomic (start)
                    ...
                    glimpse(scen_arealoss_perc_both)
                    scen_arealoss_perc_both <- scen_arealoss_perc_both %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_both, file="Results\\Area\\Proj_Area_Perc_both_SSP570_Sept2024.csv") 
                    ...
                    scen_arealoss_perc_both_countrycode <- scen_arealoss_perc_both %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_both_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_both_SSP570_Sept2024.csv")
                    glimpse(scen_arealoss_perc_both_countrycode)
                    ...
                    ## New Figure Pas + Future (start)area_change_wrt_1996_lower_limit
                        ...
                        #write.csv(summary_dfs_total_sum,file="Results\\Area\\summary_dfs_total_sum.csv")
                        write.csv(agg_aloss_neg_total_both, file="Results\\Area\\Proj_Area_Perc_Total_both_SSP570_Sept2024.csv")
...
## SSP3
    ...
    mcn <- read.csv("C:\\Users\\basti\\Documents\\GitHub\\Mangroves_ClimateChange\\Data\\mangrove_alldata_sept2023.csv")
                ...
                ## Only Climate Change (start)
                    ...
                    scen_arealoss_perc_onlyCC <- scen_arealoss_perc_onlyCC %>% filter(!is.na(gridcell_id))
                    glimpse(scen_arealoss_perc_onlyCC)
                    write.csv(scen_arealoss_perc_onlyCC, file="Results\\Area\\Proj_Area_Perc_OnlyCC_SSP370_Sept2024.csv")
                    ...
                    scen_arealoss_perc_onlyCC_countrycode <- scen_arealoss_perc_onlyCC %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlyCC_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyCC_SSP370_Sept2024.csv")
                        ...
                        write.csv(agg_aloss_neg_total_onlyCC, file="Results\\Area\\Proj_Area_Perc_Total_OnlyCC_SSP370_Sept2024.csv")
                ...
                ## Only GDP (start)
                    ...
                    glimpse(scen_arealoss_perc_onlygdp)
                    scen_arealoss_perc_onlygdp <- scen_arealoss_perc_onlygdp %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_onlygdp, file="Results\\Area\\Proj_Area_Perc_OnlyGDP_SSP370_Sept2024.csv") 
                    agg_aloss_neg_total_onlygdp <- aggregate(mangrove_area_future_loss~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_lower_limit <- aggregate(mangrove_area_future_loss_lower_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp_upper_limit <- aggregate(mangrove_area_future_loss_upper_limit~year,data=scen_arealoss_perc_onlygdp,FUN="sum")
                    agg_aloss_neg_total_onlygdp <- merge(agg_aloss_neg_total_onlygdp,agg_aloss_neg_total_onlygdp_upper_limit,by="year")
                    ...
                    scen_arealoss_perc_onlygdp_countrycode <- scen_arealoss_perc_onlygdp %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_onlygdp_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_OnlyGDP_SSP370_Sept2024.csv") 
                    glimpse(scen_arealoss_perc_onlygdp_countrycode)
                    ...
                    sum2020 <- sum(mcn_2020$mangrove_area,na.rm=T)
                        ...
                        write.csv(agg_aloss_neg_total_onlygdp, file="Results\\Area\\Proj_Area_Perc_Total_OnlyGDP_SSP370_Sept2024.csv")
                ...
                ## Both: Climate and Socioeconomic (start)
                    ...
                    glimpse(scen_arealoss_perc_both)
                    scen_arealoss_perc_both <- scen_arealoss_perc_both %>% filter(!is.na(gridcell_id))
                    write.csv(scen_arealoss_perc_both, file="Results\\Area\\Proj_Area_Perc_both_SSP370_Sept2024.csv") 
                    ...
                    scen_arealoss_perc_both_countrycode <- scen_arealoss_perc_both %>% group_by(countrycode,year) %>% 
                                                            ...
                                                            mangrove_area_future_loss_upper_limit_wrt1996= level_2020_loss+mangrove_area_future_loss_upper_limit,
                                                            mangrove_area_future_loss_wrt1996= mangrove_area_future_loss+level_2020_loss)
                    write.csv(scen_arealoss_perc_both_countrycode, file="Results\\Area\\Proj_Area_Perc_Country_both_SSP370_Sept2024.csv")
                    glimpse(scen_arealoss_perc_both_countrycode)
                    ...
                    ## New Figure Pas + Future (start)area_change_wrt_1996_lower_limit
                        ...
                        #write.csv(summary_dfs_total_sum,file="Results\\Area\\summary_dfs_total_sum.csv")
                        write.csv(agg_aloss_neg_total_both, file="Results\\Area\\Proj_Area_Perc_Total_both_SSP370_Sept2024.csv")
